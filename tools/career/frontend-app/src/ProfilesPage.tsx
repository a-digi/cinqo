  ...
  return (
    <div className="max-w-2xl p-6 font-sans text-gray-900">
      ...
      <p className="mb-5 text-sm text-gray-500">
        Each profile represents one job seeker using this tool — first name, last name, and
        external platform links (LinkedIn and so on). A profile owns one or more personas.
        Deleting a profile permanently deletes every persona it owns, and everything those
        personas own in turn (details, skills, experience) — the largest deletion this tool can
        perform in one call, with no separate confirmation step.
      </p>
            ...
            <div key={p.id} className="rounded-md border border-gray-200 p-3">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) => setLinkDraft(p.id, { ...draft, platform: e.target.value })}
                ...
                <input
                  value={draft.url}
                  onChange={(e) => setLinkDraft(p.id, { ...draft, url: e.target.value })}
                  placeholder="URL"
