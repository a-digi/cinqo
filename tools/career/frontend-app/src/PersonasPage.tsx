...
// Full persona CRUD — the natural entry point now that nothing else
// (Profile/Skills/Experience) can be used without a persona already
// existing (step 8's own "forced to be mapped" enforcement). See
...
export function PersonasPage() {
  ...
  function handleCreate() {
    ...
    setError('')
    createPersona(name, newDescription.trim() || undefined)
      .then((result) => {
        setPersonas(result)
        setNewName('')
        setNewDescription('')
  ...
  return (
    <div className="max-w-2xl p-6 font-sans text-gray-900">
      ...
      <p className="mb-5 text-sm text-gray-500">
        Each persona owns its own career profile, skills, and experience — use separate personas
        for different roles you're pursuing (e.g. "Backend Engineer", "Freelance Consultant").
        Deleting a persona permanently deletes its own profile, every skill, and every experience
        entry — there's no separate confirmation step.
      </p>
