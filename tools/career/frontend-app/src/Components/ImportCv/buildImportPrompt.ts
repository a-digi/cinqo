...
// the CV's own Media file id as an opaque "media:<fileId>" reference
// (never a fetchable URL — see api.ts's own CVUploadResult doc comment
// and conversation/chat.go's invokeToolCall, which resolves this
// server-side before pdf_to_markdown is ever invoked). Step 6 parses
// this same contract — kept here, not redefined there, since the
// instruction and the parser must agree on one shape. See
// plan/ai/tools/career/import-cv/step-05-ai-analysis-kickoff-and-polling.md
// and plan/ai/media/step-02-career-media-migration.md.
const SCHEMA = `{
...
export function buildImportPrompt(cvFileId: string): string {
  return `You are helping import a CV into this user's career profile.

1. Call pdf_to_markdown with url="media:${cvFileId}" to read the CV's text content.
